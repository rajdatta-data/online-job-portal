<?php
include("config.php");
$a="select Email_id,chk_qus,chk_ans from professional";
$b=mysqli_query($conn,$a);
while ($data=mysqli_fetch_array($b)) 
{
    $chk_email=$data['Email_id'];
    $chk_ques=$data['chk_qus'];
    $chk_ans=$data['chk_ans'];
}
?>
...
<div class="pic">
      <center> <div class="fluid-container p4 ">
        <div class="login"><br><br>
                    <center><img src="myimages/loginimg.png" class="img-circle"></center>
                    <br>
 <h2> <b>Professional<br>Forget Password</b></h2><br>
         ...
         <input type="text" name="ans"> 
            ...
            <div class="input-group">
                    ...
                    <input type="Password" name="pass"  placeholder="Enter your new Password" class="input-sm">
                </div><br>
          
                 <div class="input-group">
                    <span class="input-group-addon"><i class="glyphicon glyphicon-lock"></i></span>
                    <input type="Password" name="cnf_pass" placeholder="Enter your Confirm Password" class="input-sm">
                </div><br>
...
if(isset($_POST['submit']))
   {
    ...
    $anwser=$_POST['ans'];
    $newpass=$_POST['pass'];
    $cnfpass=$_POST['cnf_pass'];

    if ($email==$chk_email && $quetion==$chk_ques && $anwser==$chk_ans
    ) 
    {
        if ($newpass==$cnfpass) 
        {
            $qr="update professional set Password='".$newpass."' where Email_id='".$email."'";
            $query=mysqli_query($conn,$qr);
            if ($query==true) 
            {
                echo "<script>alert('Password Change successfully');</script>";
                header('location:std_professional.htm');
            }
            else
            {
                echo "<script>alert('Password Change Failed');</script>";
                 header('location:forget_professional.php');
            }
        }
    }
    }
