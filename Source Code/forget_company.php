<?php
include("config.php");
$a="select Email_id,chk_qus,chk_ans from comany";
...
<div class="pic">
      <center> <div class="fluid-container p4 ">
        <div class="login"><br><br>
                    ...
                    <br>
                    <h2><b>Company<br>Forget Password</h2></b><br>
...
if(isset($_POST['submit']))
   {
    ...
    ) 
    {
        if ($newpass==$cnfpass) 
        {
            $qr="update comany set Password='".$newpass."' where Email_id='".$email."'";
            $query=mysqli_query($conn,$qr);
            if ($query==true) 
            {
                echo "<script>alert('Password Change successfully');</script>";
              
                  echo "<script>location.replace('com_login.htm')</script>";
            }
            else
            {
                echo "<script>alert('Password Change Failed');</script>";
                 header('location:forget_company.php');
            }
        }
    }
}
